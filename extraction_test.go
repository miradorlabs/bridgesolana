...
func TestResolution_Resolve_Destination(t *testing.T) {
	...
	r := &Resolution{
		MessageProgramID: "anyProg",
		// AccountDiscriminator is zero — destination dispatch.
		correlation: []correlationField{
			{Offset: 12, Size: 8, Type: "uint64", Field: "nonce"},
		},
	}
	...
	if !matched {
		t.Fatal("destination resolve should always report matched=true on success")
	}
	if id != "12345" {
...
func TestResolution_Resolve_DestinationShortData(t *testing.T) {
	...
	if err == nil {
		t.Fatal("expected error for truncated destination data")
	}
	if matched {
		t.Fatal("matched should be false on error")
	}
}
...
func TestResolution_Resolve_KeccakRoundTrip(t *testing.T) {
	...
	data := append(append(append([]byte{}, anchorDisc...), vecLen...), msg...)
